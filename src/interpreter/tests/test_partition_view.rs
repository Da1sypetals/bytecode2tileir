...
fn test_dim_map_2d_transpose() {
    let row = 8;
    let col = 8;
    let mut buffer = vec![0u8; row * col * 2];
    ...
    for i in 0..8 {
        for j in 0..8 {
            unsafe {
                let idx = i * 8 + j;
                *(ptr.add(idx * 2) as *mut f16) = (i * 20 + j) as f16;
            }
        }
    }

    let tensor_view = TensorView::new(ptr, ElemType::F16, vec![8, 8], vec![8, 1]);
    let partition = PartitionView::new(tensor_view, vec![2, 2], vec![1, 0], false, None);

    let tile = partition.load_tile(&[1, 3]);

    assert_eq!(tile.get_scalar(&[0, 0]), Scalar::F16(122.0));
    assert_eq!(tile.get_scalar(&[0, 1]), Scalar::F16(142.0));
    assert_eq!(tile.get_scalar(&[1, 0]), Scalar::F16(123.0));
    assert_eq!(tile.get_scalar(&[1, 1]), Scalar::F16(143.0));
}
